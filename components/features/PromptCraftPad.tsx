...
export const PromptCraftPad: React.FC = () => {
    ...
    const variableNames = useMemo(() => {
        ...
        const uniqueNames = new Set([...activePrompt.text.matchAll(/\{(\w+)\}/g)].map(match => match[1]));
    ...
    useEffect(() => {
        ...
        if (activePrompt) {
            const currentInList = prompts.find((p: Prompt) => p.id === activePrompt.id);
            if (!currentInList) {
    ...
    const handleNameUpdate = (id: number, newName: string) => {
        ...
        if (trimmedName) {
            setPrompts(prompts.map((p: Prompt) => p.id === id ? {...p, name: trimmedName} : p));
        }
        setEditingId(null);
    };
                    ...
                    <ul className="space-y-2 flex-grow overflow-y-auto pr-2">
                        {prompts.map((p: Prompt) => (
                            <li key={p.id} className="group flex items-center justify-between">
                                <div 
                                    className={`w-full text-left rounded-md cursor-pointer ${activePrompt?.id === p.id ? 'bg-primary/10' : 'hover:bg-gray-100 dark:hover:bg-slate-700'}`}
                                    onClick={() => { if (editingId !== p.id) setActivePrompt(p) }} 
                                ...
                                >
                                    {editingId === p.id ? (
                                        <input
                                            ...
                                            onKeyDown={e => { if (e.key === 'Enter') handleNameUpdate(p.id, tempName) }}
                                            className="bg-background text-text-primary w-full px-3 py-2 rounded-md"
                                            onClick={e => e.stopPropagation()}
                                        />
                                    ) : (
                                        <div className={`w-full text-left px-3 py-2 ${activePrompt?.id === p.id ? 'text-primary font-semibold' : ''}`}>
                                            {p.name}
                                        </div>
                                    )}
                                </div>
                                <button 
                                    ...
                                    className="ml-2 p-1 text-text-secondary hover:text-red-500 opacity-0 group-hover:opacity-100"
